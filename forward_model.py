...
def scale_rate_by_node(baseline_k: float, log2fc: float,
    ...
    coupling in [0,1] is a deliberately conservative fudge factor: transcript
    change rarely translates 1:1 to enzyme activity / flux. Report the number
    AND the coupling you assumed. Do not pretend coupling = 1 is measured truth.
    """
    return baseline_k * (2.0 ** log2fc) ** coupling
...
if __name__ == "__main__":
    p = HPParams()
    # toy example: an LDHA knockdown, log2fc = -1.0 (halving), coupling 0.7
    kPL_new = scale_rate_by_node(p.kPL, log2fc=-1.0, coupling=0.7)
    r = detectability(p.kPL, kPL_new, "kPL")
    print(f"baseline kPL={r.k_baseline:.4f}  perturbed={r.k_perturbed:.4f}")
    print(f"AUC ratio baseline={lac_pyr_ratio_auc(p):.3f} "
          f"perturbed={lac_pyr_ratio_auc(p, kPL=kPL_new):.3f}")
    print(r.note)
